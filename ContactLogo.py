...
local_url = "https://odoouat.surfacemeasurements.com"
    ...
    for product in productData:
        ...
        if product:
            ...
            if product['default_code'] and product['default_code'] in product_wizard_dict.keys():
                ...
                product['Specification'] = "<pre>{}</pre>".format(product_wizard_dict[product['default_code']]['notes_specs'])
...
def remove_customer_vendor_tax(productData):
    for product in productData:
        product_id, productId = False, False

        if product['id'] in old_id_mapping['product.template'].keys():
            product_id = old_id_mapping['product.template'][product['id']]
        else:
            productId = get_local_model_data(local_socket, 'product.template', 'search_read', 0, 100, 100,
                                              [('old_id', '=', product['id'])])

        if product_id or productId:
            p = write_record('product.template', local_url, local_database, local_uid, local_password, {'taxes_id':[(6,0,[])], 'supplier_taxes_id':[(6,0,[])]},
                                 product or productId[0]['id'])
...
def set_vendor_logo(vendors, productImages):
    for productdata in productImages:
        currentVendor = list(filter(lambda vendor: vendor if vendor['old_id'] == productdata['id'] else False, vendors))[0]
        p = write_record('res.partner', local_url, local_database, local_uid, local_password,
                         {'image_1920':productdata['image'], 'image_1024':productdata['image'], 'image_128':productdata['image'], 'image_256':productdata['image'],  'image_512':productdata['image'], 'image_medium':productdata['image']},
                         currentVendor['id'])

        if p:
            print("Vendor images Updated for id %s"%(currentVendor['id']))
        else:
            print("Vendor logo updation failed")
...
contacts = get_local_model_data(local_socket, 'res.partner', 'search_read', 0, 100, 100,
                                              [('old_id', '!=', False)])
print(len(contacts))
contact_images = get_model_data(sms_model_socket, 'res.partner', 'search_read', 0, 100, 100,
                                     [['image','!=',False],['id', 'in', [x['old_id'] for x in contacts]]], ['image'])
print(len(contact_images))
set_vendor_logo(contacts, contact_images)
