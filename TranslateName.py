...
def write_record(model, local_url, local_database, local_uid, local_password, data, id, lang='en_GB'):
    models = xmlrpclib.ServerProxy('{}/xmlrpc/2/object'.format(local_url))
...
def get_model_data(model_arg, model, operation, offset, limit, breakLimit, condition, fields, lang = 'en_GB'):
    try:
        model_data = []
        if not fields:
        ...
        while True:
            ...
            data = model_arg.execute_kw(sms_database, sms_uid, sms_password,
                              model, operation,
                              [condition],
                              {'fields': fields,'context':{'lang':lang},'offset':offset ,'limit': limit})
...
def create_update_local_products(product_data, language = "en_GB"):
    for prod in product_data:
        try:
            ...
            if product_id:
                p = write_record('product.template', local_url, local_database, local_uid, local_password,
                                 {'name': prod['name']},
                                 product_id[0]['id'], language)
                if p:
...
for language in language_list:
    products = get_model_data(sms_model_socket, 'product.template', 'search_read', 0, 100, 100, [],
                                 ['name'], language)
    p = create_update_local_products(products, language)
