...
def update_product_internal_ref(productData):
    for product in productData:
        try:
            if product['default_code']:
                id = False
                if product['id'] in old_id_mapping['product.template'].keys():
                    id = old_id_mapping['product.template'][product['id']]
                else:
                    product_id = get_local_model_data(local_socket, 'product.template', 'search_read', 0, 100, 100,
                                                      [('old_id', '=', prod['id'])])
                    id = product_id[0]['id']
                product['default_code'] = product['default_code'].replace('Rev.','Akarigo')
                product['default_code'] = product['default_code'].replace('Rev','Akarigo')
                update_dict = {'default_code':False, 'version':False}
                update_dict['default_code'], update_dict['version'] = product['default_code'].split('Akarigo')[0], product['default_code'].split('Akarigo')[1] if len( product['default_code'].split('Akarigo')) > 1 else 0
                print(product['default_code'])
                update_dict['version'] = int(float(update_dict['version']))
                print(update_dict)
                p = write_record('product.template', local_url, local_database, local_uid, local_password, update_dict,
                                 id)

                if p:
                    print("Product %s updated"%(old_id_mapping['product.template'][product['id']]))
                else:
...
update_product_internal_ref(productData)
